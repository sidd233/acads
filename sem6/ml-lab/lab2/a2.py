...
data = np.random.randint(1,100,(500,3))
df = pd.DataFrame(data,columns=["ID","Flat","House"])
df["Purchases"] = 6*df["Flat"] + 8*df["House"] + np.random.randint(0,500,500)

df = df.sample(frac=1).reset_index(drop=True)

X = df[["Flat","House"]].values
Y = df["Purchases"].values.reshape(-1,1)
